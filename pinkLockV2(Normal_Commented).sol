...
contract PinkLock02 is IPinkLock, Pausable {
    ...
    function unlock(uint256 lockId) external override validLock(lockId) {
        // Lock storage userLock = _locks[_getActualIndex(lockId)]; // original
    ...
    function editLockDescription(uint256 lockId, string memory description)
        ...
        validLock(lockId)
    {
        // Lock storage userLock = _locks[_getActualIndex(lockId)]; //orignal
        Lock storage userLock = _locks[lockId]; //modified
        require(
            userLock.owner == msg.sender,
            "You are not the owner of this lock"
    ...
    function totalTokenLockedCount() external view returns (uint256) {
        // return allLpTokenLockedCount() + allNormalTokenLockedCount(); //original
        return allNormalTokenLockedCount(); //modified
    }
    ...
    function totalLockCountForUser(address user)
        ...
        returns (uint256)
    {
        // return normalLockCountForUser(user) + lpLockCountForUser(user); //orignal
        return normalLockCountForUser(user); //modified.
    }
    ...
    function _getActualIndex(uint256 lockId) internal view returns (uint256) {
        ...
        // }
        // uint256 actualIndex = lockId - ID_PADDING; //original
